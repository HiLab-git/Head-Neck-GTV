...
# if the pixel spacing in z direction is 3mm
    # offset_d  = [-3, 3,  0, 0,  0, 0]
    # offset_h  = [ 0, 0, -1, 1,  0, 0]
    # offset_w  = [ 0, 0,  0, 0, -1, 1]
...
def binary_volume(s_volume, g_volume):
    s_v = s_volume.sum()
    g_v = g_volume.sum()
    assert(g_v > 0)
    return s_v
...
def get_evaluation_score(s_volume, g_volume, spacing, metric):
    ...
    elif(metric_lower == "vvc"):
        score = binary_volume(s_volume, g_volume)
    else:
