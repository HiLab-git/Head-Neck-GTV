...
def resize_ND_volume_to_given_shape(s, g, order):
    ...
    min_edge = min(input_shape[0:])
    # output_size = [output_size * input_shape[i] / min_edge for \
    #                 i in range(input_dim)]
    scale = [(output_size[i])/input_shape[0:][i] for i in range(input_dim)]
